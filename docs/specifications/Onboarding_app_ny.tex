...
\begin{frame}[label=intro]
  ...
  % SEKTION 1B: ORGANISATIONSNUMMER (FLYTTAT FRÅN RISKFRÅGOR STEG 1)
  % CRITICAL CHANGE 2025-11-21: Orgnr måste samlas in FÖRE Riskfrågor
  % Anledning: Backend behöver skapa företagsmapp (data/{user_id}/{orgnr}/) 
  % direkt vid Uppdragsval för att möjliggöra "Parkera och Avsluta" funktionalitet
  \begin{block}{\textbf{1B. Företagets organisationsnummer}* \hspace{0.3cm} ⓘ}
    \footnotesize
  ...
  \textbf{Sök företagsnamn:} \underline{\hspace{6cm}} (autocomplete)\\[0.2cm]
  
  \textbf{Organisationsnummer:}* \underline{\hspace{4cm}} (NNNNNN-NNNN)
  
  \begin{block}{\textbf{Autocomplete-logik}}
    \tiny
    \textbf{Exempel:}
    \begin{itemize}
      \item Användaren skriver "Celestial Redo"
      \item Dropdown visar: "Celestial Redovisning AB (556903-8671)"
      \item Väljer från listan → Org.nr fylls i automatiskt
    \end{itemize}
    \vspace{0.1cm}
    
    \tiny \textbf{Teknisk implementation:}\\[0.1cm]
    \tiny • Fuzzy search i lokal Bolagsverket-FIL (600 000+ företag, <1ms lookup)\\[0.1cm]
    \tiny • Autocomplete-komponent (React-Select eller Material-UI)\\[0.1cm]
    \tiny • Organisationsnummer: förifyllt från autocomplete, kan redigeras manuellt\\[0.2cm]
    
    \tiny \textbf{UX-logik:}\\[0.1cm]
    \tiny • Om användaren RADERAR företagsnamnet → RENSA även org.nr\\[0.1cm]
    \tiny • Om användaren ÄNDRAR org.nr manuellt → BEHÅLL det (manuell override)\\[0.1cm]
    \tiny • Grön bakgrund (bg-brand-50) när org.nr är förifyllt från autocomplete\\[0.2cm]
    
    \tiny \textbf{Hittar du inte ditt företag?}\\[0.1cm]
    \tiny → Skriv in organisationsnummer manuellt (valideras mot Bolagsverket API)
  \end{block}
  
  \begin{alertblock}{Expansionsknapp (ⓘ) → Varför samlar vi in detta?}
    \tiny \textbf{Arkitektonisk anledning:}\\[0.1cm]
    \tiny Organisationsnummer krävs för att skapa företagsmappen i backend-systemet\\[0.1cm]
    \tiny (\texttt{data/\{user\_id\}/\{orgnr\}/}). Detta möjliggör:\\[0.2cm]
    
    \tiny • \textbf{"Parkera och Avsluta"} – Spara onboarding-session för senare\\[0.1cm]
    \tiny • \textbf{Partiell datainsamling} – Spara progress även om användaren avbryter\\[0.1cm]
    \tiny • \textbf{Strukturerad datalagring} – Alla företagsdata organiseras per orgnr\\[0.2cm]
    
    \tiny \textbf{Juridisk grund (Penningtvättslagen):}\\[0.1cm]
    \tiny Organisationsnummer är grunden för identifiering av juridisk person enligt\\[0.1cm]
    \tiny \textbf{3 kap. 7 § PTL} och \textbf{01FS 2024:20, 3 kap. 3 §}.\\[0.2cm]
    
    \tiny Vi måste kontrollera identiteten genom oberoende källor (Bolagsverket =\\[0.1cm]
    \tiny offentligt register = tillförlitlig källa). Denna kontroll måste dokumenteras\\[0.1cm]
    \tiny och sparas, oavsett om vi känner kunden sedan tidigare.
  \end{alertblock}
...
\begin{frame}[label=riskfragor-steg1]
  ...
  % ORGANISATIONSNUMMER HAR FLYTTATS TILL UPPDRAGSVAL-SLIDE (2025-11-21)
  % Företagets namn visas här för bekräftelse (förifyllt från Uppdragsval)
  \textbf{2. Bekräfta företagets information}* \hspace{0.3cm} ⓘ\\
  \vspace{0.2cm}
  
  \begin{block}{Företagsinformation (förifylld från Uppdragsval)}
    \tiny \textbf{Företagsnamn:} \underline{\hspace{6cm}} (förifyllt, ej redigerbart)\\[0.2cm]
    \tiny \textbf{Organisationsnummer:} \underline{\hspace{4cm}} (förifyllt, ej redigerbart)\\[0.2cm]
  \end{block}
  
  \begin{block}{\textbf{ARKITEKTONISK ÄNDRING (2025-11-21)}}
    \tiny
    \textbf{Organisationsnummer har flyttats till Uppdragsval (första steget).}\\[0.2cm]
    
    \tiny \textbf{Anledning:}\\
    \tiny • Backend måste skapa företagsmapp (\texttt{data/\{user\_id\}/\{orgnr\}/}) tidigt\\
    \tiny • Möjliggör "Parkera och Avsluta"-funktionalitet\\
    \tiny • Tillåter sparning av partiell progress vid avbrott\\
    \tiny • Strukturerad datalagring från start av onboarding\\[0.2cm]
    
    \tiny \textbf{Användarupplevelse:}\\
    \tiny Användaren behöver inte fylla i organisationsnummer igen här.\\
    \tiny Fälten är förifyllda från Uppdragsval och visas endast för bekräftelse.\\[0.2cm]
    
    \tiny \textbf{Om fel företag valts:}\\
    \tiny → Gå tillbaka till Uppdragsval via sidebar för att ändra organisationsnummer.
  \end{block}
  \begin{alertblock}{Expansionsknapp (ⓘ) → Lagtext}
...
% NYTT 2025-11-22: localStorage vs Backend - Data Persistence Strategy
\begin{frame}[label=localstorage-strategy]
  \frametitle{Data Persistence: localStorage vs Backend (Uppdaterad 2025-11-22)}

  \footnotesize

  \begin{alertblock}{Arkitektonisk Princip}
    \tiny \textbf{Backend är Source of Truth} – localStorage används endast som cache/draft buffer
  \end{alertblock}

  \vspace{0.3cm}

  \textbf{localStorage SKA lagra (user-scoped):}\\
  \begin{itemize}
    \tiny
    \item \textbf{Session tokens:} \texttt{accessToken}, \texttt{refreshToken} (JWT authentication)
    \item \textbf{Draft changes:} \texttt{onboarding-\{userId\}-draft-steg2} (temporära formulärändringar)
    \item \textbf{UI preferences:} \texttt{sidebar-collapsed}, \texttt{theme} (användarinställningar)
    \item \textbf{Performance cache:} \texttt{cache-companies-\{userId\}} (kopia av backend-data, TTL 5-10 min)
  \end{itemize}

  \vspace{0.3cm}

  \textbf{localStorage SKA INTE lagra:}\\
  \begin{itemize}
    \tiny
    \item \textcolor{red}{❌} Source of truth för onboarding-data (Backend äger: \texttt{data/\{user\_id\}/\{orgnr\}/})
    \item \textcolor{red}{❌} PII utan kryptering (personnummer, företagsdata → Backend)
    \item \textcolor{red}{❌} Permanent företagsdata (Backend JSON-filer eller PostgreSQL)
    \item \textcolor{red}{❌} Multi-device synkad data (Backend sync via GET /resume)
  \end{itemize}

  \vspace{0.3cm}

  \textbf{Dataflöde (ny arkitektur):}\\
  \begin{enumerate}
    \tiny
    \item \textbf{User fyller formulär} → onChange sparar till localStorage (draft)
    \item \textbf{User klickar "Nästa"} → POST till backend → Backend sparar JSON → localStorage draft rensas
    \item \textbf{User loggar ut} → localStorage: tokens behålls, draft kan finnas kvar
    \item \textbf{User loggar in igen} → GET /api/onboarding/list → Backend returnerar företag → Resume-dialog
    \item \textbf{User klickar "Fortsätt"} → GET /api/onboarding/resume/\{orgnr\} → Backend läser JSON → Frontend populerar formulär
  \end{enumerate}

  \vspace{0.3cm}

  \begin{block}{Multi-Device Support}
    \tiny Med backend som source of truth kan användare börja på mobil, fortsätta på desktop.\\
    localStorage används endast för snabbare laddning (cache), EJ för persistent lagring.
  \end{block}
\end{frame}


% NYTT 2025-11-22: Session Management - Resume Functionality
\begin{frame}[label=session-resume]
  \frametitle{Session Management: Resume \& Multi-Company (Ny 2025-11-22)}

  \footnotesize

  \textbf{Vid login: Frontend frågar backend om pågående onboardings}\\
  \tiny GET /api/onboarding/list → \texttt{\{ companies: [\{orgnr, companyName, currentStep, progress\}] \}}

  \vspace{0.3cm}

  \textbf{Resume-Dialog (OnboardingResumeDialog.jsx):}\\
  \begin{itemize}
    \tiny
    \item \textbf{Fortsätt-knapp:} Laddar företagsdata (GET /resume/\{orgnr\}) → Navigerar till \texttt{currentStep}
    \item \textbf{Radera-knapp:} DELETE /api/onboarding/delete/\{orgnr\} → Tar bort från lista
    \item \textbf{Ny Onboarding Session:} Stänger dialog → Navigerar till Uppdragsval (tom)
  \end{itemize}

  \vspace{0.3cm}

  \textbf{Exempel-scenario (konsult med flera kunder):}\\
  \begin{enumerate}
    \tiny
    \item Login → Resume-dialog visar: "Övningsbolag1 AB" (45\%), "Övningsbolag2 AB" (20\%)
    \item Klickar "Fortsätt" för Övningsbolag1 → Alla fält förifyllda från backend
    \item Arbetar till lunch → Klickar "Logga ut" (data sparad i backend)
    \item Login igen → Samma dialog, klickar "Ny Onboarding Session"
    \item Påbörjar "Övningsbolag3 AB" → Nästa login: 3 företag i listan
  \end{enumerate}

  \vspace{0.3cm}

  \textbf{Progress Calculation:}\\
  \tiny Backend beräknar progress baserat på vilka JSON-filer som existerar:\\
  \texttt{metadata.json (15\%) + företagsinfo1\_static\_KYC.json (30\%) + företagsinfo2\_static\_KYC.json (40\%) + övriga (15\%)}

  \vspace{0.3cm}

  \begin{alertblock}{Header-knapp: "Avsluta (rensar)"}
    \tiny I profil-dropdown: Raderar ALLA pågående onboardings (loop DELETE), rensar localStorage, loggar ut.\\
    Används när användare vill starta helt om från början.
  \end{alertblock}
\end{frame}
