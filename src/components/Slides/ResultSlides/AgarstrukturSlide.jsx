      ...
      <div className="bg-white rounded-card shadow-2xl p-8 max-w-4xl w-full">
          ...
          <div className="mb-6 p-6 bg-brand-50 rounded-card border border-brand-200">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-start">
                ...
                <span className="text-sm text-gray-800 font-semibold text-right">{beneficialOwner.name}</span>
              </div>
              <div className="flex justify-between items-start">
                <span className="text-sm text-gray-600 font-medium">Personnummer:</span>
                <span className="text-sm text-gray-800 font-mono text-right">{beneficialOwner.personalNumber}</span>
              </div>
              <div className="flex justify-between items-start">
                <span className="text-sm text-gray-600 font-medium">Ägarandel:</span>
                  <span className="text-brand-700 text-subsection-title font-semibold text-right">{beneficialOwner.ownershipPercent}%</span>
              </div>
              <div className="flex justify-between items-start">
                <span className="text-sm text-gray-600 font-medium">Kontrollmekanism:</span>
                <span className="text-sm text-gray-800 text-right">{beneficialOwner.controlType}</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="mb-6 p-6 bg-white rounded-card border border-brand-100">
            ...
            {ownerData.owners.map((owner, index) => (
              <div key={index} className="p-4 bg-brand-50 rounded-box border border-brand-200 hover:shadow-md transition-shadow">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-semibold text-gray-800">{owner.name}</span>
          ...
          <div className="mt-4 pt-4 border-t border-brand-200">
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-700 font-semibold">Totalt:</span>
        ...
        {alternativeBeneficialOwners && alternativeBeneficialOwners.length > 0 && (
          <div className="mb-8 p-6 bg-brand-50 rounded-card border border-brand-200">
            <h2 className="text-subsection-title text-brand-900 mb-3 flex items-center">
