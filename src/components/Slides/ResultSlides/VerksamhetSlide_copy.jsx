        ...
        <div className="mb-6 p-6 bg-gradient-to-r from-brand-50 to-brand-50 rounded-card border border-brand-200">
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between items-start">
              ...
              <span className="text-sm text-gray-800 font-mono text-right">{data.organizationNumber}</span>
            </div>
            <div className="flex justify-between items-start">
              <span className="text-sm text-gray-600 font-medium">Registreringsdatum:</span>
              <span className="text-sm text-gray-800 text-right">{data.registrationDate}</span>
            </div>
            <div className="flex justify-between items-start">
              <span className="text-sm text-gray-600 font-medium">Status:</span>
              <span className="text-sm text-green-600 font-semibold text-right">{data.status}</span>
            </div>
          </div>
        </div>
            ...
            {data.sniCodes && data.sniCodes.map((sni, index) => (
              <div key={index} className="p-4 bg-white rounded-box border border-brand-200">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm text-gray-600 font-medium">SNI-kod:</span>
                  <span className="text-gray-800 font-mono text-subsection-title font-semibold">{sni.code}</span>
                </div>
                <p className="text-sm text-gray-700">{sni.description}</p>
              </div>
            ))}
            {data.secondaryNames && data.secondaryNames.length > 0 && (
              <div className="mt-4 pt-4 border-t border-brand-200">
                <span className="text-sm text-gray-600 font-medium block mb-2">Alternativa namn:</span>
